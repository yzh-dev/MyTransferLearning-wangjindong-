    ...
    def finetune(self, trainloader, testloader, optimizer, nepochs=10, save_path=None,schuse=None):
        ...
        for epoch in range(nepochs):
            ...
            for batch in tqdm(trainloader):
                ...
                # -----------------------------------------------------------
                # 原始步骤，直接通过视觉适配器和文本适配器
                # logits_per_image, logits_per_text = self.model(image, text)

                # -----------------------------------------------------------
                # 添加了1个步骤：经过适配器
                image_features = self.model.encode_image(image)
                x = self.ImgAdapter(image_features)
                ratio = 0.2
                image_features = ratio * x + (1 - ratio) * image_features  # 与原始特征进行融合
                text_features = self.model.encode_text(text)
                ...
                # label_features = self.get_text_features_list(labels)  # batch_size*1024
                # image_features = self.get_image_features(image)  # batch_size*1024
                # similarity = self.get_similarity(image_features, label_features)  # batch_size*batch_size
                ...
                # epoch_train_acc = np.mean(np.array(res)[:, 0] == np.array(res)[:, 1])
                ...
                # 由于我们不需要更新CLIP模型的参数，因此我们只需要更新适配器的参数即可
                optimizer.step()
                # if self.device == "cpu":
                #     optimizer.step()
                # else:
            ...
            self.logger.info("Epoch {} : Loss {}, Acc {:.4f}".format(
                epoch, total_loss / len(trainloader), eval_acc))
        return best_acc
